...
export default defineComponent({
  ...
  setup(_, ctx) {
    ...
    const edit_user = async (val: {
      id: String;
      name: String;
      quantity_users: Number;
      id_flats: String;
    }) => {
      const url = `${ConfigVue.url_server}/routers/house_settlement_money/renting_user/renting_user/edit_user`;
      const method = "PUT";
      const headers = {
        "Content-Type": "application/json",
        authorization: `Bearer ${store.getters["auth/getTokens"].access_token}`,
    ...
    return { load_flats, add_user, load_rent_users, delete_user, edit_user };
  },
});
</script>
